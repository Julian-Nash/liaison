...
def test_field():
    field = Field(type=str, input_types=(str,))
...
def test_field_raises_exception_incorrect_type():
    ...
    field = Field(type=int, input_types=(int, str))
    with pytest.raises(ValidationError):
        value = field.validate("foo", "bar")


def test_field_casts_to_type():
    field = Field(type=str, input_types=(str, int, float))
    value = field.validate("foo", 1)
    assert value == "1"


def test_field_raises_exception_with_strict_type():
    field = Field(type=int, input_types=(str, int, float), strict_type=True)
    with pytest.raises(ValidationError):
        value = field.validate("foo", "1")


def test_field_with_required():
    field = Field(type=str, input_types=(str,), required=True)
    value = field.validate("foo", "bar")
    assert value == "bar"
...
def test_field_with_default():
    field = Field(type=str, input_types=(str,), default="DEFAULT")
    value = field.validate("foo", None)
    assert value == "DEFAULT"


def test_field_with_callable_default():
    field = Field(type=str, input_types=(str,), default=lambda: "DEFAULT")
    value = field.validate("foo", None)
    assert value == "DEFAULT"


def test_field_with_choices():
    field = Field(type=str, input_types=(str,), choices=["Hello", "World"])
...
def test_field_with_validator():
    ...
    field = Field(type=str, input_types=(str,), validator=my_validator)
...
def test_field_with_incorrect_validator_type():
    with pytest.raises(TypeError):
        field = Field(type=str, input_types=(str,), validator="foo")
